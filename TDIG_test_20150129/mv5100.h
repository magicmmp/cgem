...
#define SM_INT_ARG2 	(VME_LM_MSTR_BUS +(sysProcNumGet() * VME_A32_REG_SIZE))
...
#define OFFBRD_VME_SEM_REG1	(VME_LM_MSTR_LOCAL + \
...
/*
 * VME MASTER WINDOW FOR UNIV II LM (MAILBOX) REGISTERS
 *
 * Universe PCI slave (VME master) window 0
 * 
 * Map access to A32 VMEbus (VME LM Regs) (64K)
 */

#define VME_LM_SLV_SIZE		0x00001000	/* this boards reg space 4KB */
#define	VME_LM_MSTR_SIZE	0x00010000	/* total system reg space 64KB*/

#define VME_A32_REG_SIZE    VME_LM_SLV_SIZE   /* individual reg space */
#define VME_A32_REG_SPACE   VME_LM_MSTR_SIZE  /* total reg space */
...
#  define VME_LM_MSTR_LOCAL	0xf0000000
#  define VME_LM_MSTR_BUS	0x40000000
#  define VAL_LSI0_BS_VALUE	(VME_LM_MSTR_LOCAL - PCI_MSTR_MEMIO_LOCAL)

#else			/* EXTENDED_VME values */

/*
 * Map access to A32 VMEbus (LM Regs) - 64K  MAP FOR EXTENDED_VME map vxWorks
 * This maps: MPU RANGE:     0xf1000000 - 0xf100ffff
 *        to: PCI RANGE:     0xf1000000 - 0xf100ffff
 *        to: VME RANGE:     0xfb000000 - 0xfb00ffff (for legacy compatibility)
 */

#  define VME_LM_MSTR_LOCAL	0xfb000000
#  define VME_LM_MSTR_BUS	0xfb000000
 ...
 *    PCI_DYNAMIC_MEM_LOCAL (no overlap of A32 and dynamic PCI memory spaces)
...
#if  ((VME_A32_MSTR_BUS + VME_A32_MSTR_SIZE) > VME_LM_MSTR_BUS) && \
      (VME_A32_MSTR_BUS < (VME_LM_MSTR_BUS + VME_LM_MSTR_SIZE))
...
/******************************************************added by taon in 2004-02-25********************************/
/*
 * VME MASTER WINDOW FOR CSR SPACE
 *
 *
 */
...
/*
 * Map access to CSR VMEbus - 16M       MAP FOR PReP vxWorks
 * This maps: MPU RANGE:     0xe1000000 - 0xe1ffffff
 *        to: PCI RANGE:     0x21000000 - 0x21ffffff
 *        to: VME RANGE:     0xff000000 - 0xffffffff
 */

#  define VME_CSR_MSTR_LOCAL   0xe1000000
#  define VAL_LSI4_BS_VALUE    (VME_CSR_MSTR_LOCAL - PCI_MSTR_MEMIO_LOCAL)
#else                           /* EXTENDED_VME values */

/*
 * Map access to CSR VMEbus - 16M       MAP FOR EXTENDED_VME map vxWorks
 * This maps: MPU RANGE:     0x30000000 - 0x30ffffff
 *        to: PCI RANGE:     0x30000000 - 0x30ffffff
 *        to: VME RANGE:     0x30000000 - 0x30ffffff
 */

#  define VME_CSR_MSTR_LOCAL 0x30000000
#  define VAL_LSI4_BS_VALUE     (VME_CSR_MSTR_LOCAL)

#if  ((VME_CSR_MSTR_BUS + VME_CSR_MSTR_SIZE) > VME_LM_MSTR_BUS) && \
      (VME_CSR_MSTR_BUS < (VME_LM_MSTR_BUS + VME_LM_MSTR_SIZE))
#  warning VME CSR space overlaps VME LM/SEM (MAILBOX) REG space
#endif
/*#  if  ((VME_CSR_MSTR_LOCAL + VME_CSR_MSTR_SIZE) > VME_A24_MSTR_LOCAL)
 *#    error VME CSR space extends into the A24 memory space
 *#  endif
 */

#if  ((VME_A32_MSTR_LOCAL + VME_A32_MSTR_SIZE) > VME_CSR_MSTR_LOCAL)
#    error VME A32 space extends into the CSR memory space
#  endif

#endif /* EXTENDED_VME */
#if (VME_CSR_MSTR_SIZE > 0x01000000)
#  error Maximum VME_ACSR_MSTR_SIZE cannot exceed 16 MB
#endif
#define VAL_LSI4_BS     (VAL_LSI4_BS_VALUE)
#define VAL_LSI4_BD     (VAL_LSI4_BS + VME_CSR_MSTR_SIZE)
#define VAL_LSI4_TO     (0x30000000 + VME_CSR_MSTR_BUS - VAL_LSI4_BS)
#define VAL_LSI4_CTL    ( LSI_CTL_EN     | LSI_CTL_WP  |\
                          LSI_CTL_D64    | LSI_CTL_CSR |\
                          LSI_CTL_DATA   | LSI_CTL_SUP |\
                          LSI_CTL_SINGLE | LSI_CTL_PCI_MEM )


/***********************************************ended by taon in 2003-11-07********************************/
...
/* Abort interrupt level */
#define ABORT_LVL		( 0x0d + EXT_INTERRUPT_BASE )

/* RTC - Alarm interrupt level */
#define RTC_ALARM_LVL		( 0x0e + EXT_INTERRUPT_BASE )
