...
void send_GEMROC_CFG_CMD_PKT( unsigned int TARGET_GEMROC_ID_param, char*COMMAND_STRING_PARAM, 
...
void send_TIGER_GCFG_Reg_CMD_PKT(unsigned int TIGER_ID_param, char*COMMAND_STRING_PARAM, void*array_to_send_param, int cmdlen,
...
sendto(socket_descriptor,buff2,cmdlen,0,(struct sockaddr *)&sin_dest,sin_len);
num++;
j=sprintf(cmd_message,"\nnum %3d ,send_TIGER_GCFG_Reg_CMD_PKT,TIGER_ID:%d\n",num,TIGER_ID_param); // C4996
fwrite(cmd_message,sizeof(unsigned char),j,fw);
j=0;
for(i=0;i<cmdlen;i++)
j+=sprintf(cmd_message+j,"%02X",buff2[i]);
...
void send_TIGER_Ch_CFG_Reg_CMD_PKT(unsigned int TIGER_ID_param, char*COMMAND_STRING_PARAM,
...
sendto(socket_descriptor,buff2,cmdlen,0,(struct sockaddr *)&sin_dest,sin_len);
num++;
j=sprintf(cmd_message,"\nnum %3d ,send_TIGER_Ch_CFG_Reg_CMD_PKT,TIGER_ch:%d\n",num,TIGER_ID_param); // C4996
fwrite(cmd_message,sizeof(unsigned char),j,fw);
j=0;
for(i=0;i<cmdlen;i++)
j+=sprintf(cmd_message+j,"%02X",buff2[i]);
...
{  
    ...
    send_GEMROC_CFG_CMD_PKT( GEMROC_ID_param,"CMD_GEMROC_LV_IVT_READ",gemroc_command_words,sizeof(gemroc_command_words), DEST_IP_ADDRESS, DEST_PORT_NO );
}
...
void send_GEMROC_LV_CMD (unsigned int GEMROC_ID_param, char*COMMAND_STRING_PARAM)
{
    ...
    gemroc_update_command_words();
    send_GEMROC_CFG_CMD_PKT( GEMROC_ID_param, COMMAND_STRING_PARAM,gemroc_command_words,sizeof(gemroc_command_words), DEST_IP_ADDRESS, DEST_PORT_NO );  }
                            ...
                            unsigned int FEB1_TDly, unsigned int FEB0_TDly)
{
    ...
    send_GEMROC_LV_CMD(GEMROC_ID_param,"CMD_GEMROC_LV_CFG_WR");  }
void send_GEMROC_DAQ_CMD(unsigned int GEMROC_ID_param,char*gemroc_DAQ_inst_param, char*COMMAND_STRING_PARAM)
{
    DAQ_set_target_GEMROC(GEMROC_ID_param);
    DAQ_set_gemroc_cmd_code(COMMAND_STRING_PARAM,1);
    DAQ_update_command_words();
    send_GEMROC_CFG_CMD_PKT( GEMROC_ID_param, COMMAND_STRING_PARAM, DAQ_command_words,sizeof(DAQ_command_words), DEST_IP_ADDRESS, DEST_PORT_NO );}

void  ResetTgtGEMROC_ALL_TIGER_GCfgReg(unsigned int GEMROC_ID_param, char*gemroc_DAQ_inst_param)
{
    send_GEMROC_DAQ_CMD(GEMROC_ID_param, gemroc_DAQ_inst_param, "CMD_GEMROC_DAQ_TIGER_GCFGREG_RESET");
    printf("\n CMD_GEMROC_DAQ_TIGER_GCFGREG_RESET\n");
    sleep(5);
}
...
void WriteTgtGEMROC_TIGER_GCfgReg_fromfile(char*GCFGReg_setting_inst, unsigned int GEMROC_ID_param, unsigned int TIGER_ID_param,
                                            char* GCFGReg_def_fname_param, char*log_fname_param)
{
    ...
    send_TIGER_GCFG_Reg_CMD_PKT( TIGER_ID_param, "WR", g_reg_command_words,sizeof(g_reg_command_words),DEST_IP_ADDRESS, DEST_PORT_NO, log_fname_param);
}

void set_FE_TPEnable(char*GCFGReg_setting_inst, unsigned int GEMROC_ID_param, unsigned int TIGER_ID_param,
                      unsigned int FE_TPEnable_param, char*log_fname_param)
{
    ...
    g_reg_update_command_words();
    send_TIGER_GCFG_Reg_CMD_PKT( TIGER_ID_param, COMMAND_STRING,g_reg_command_words,sizeof(g_reg_command_words),DEST_IP_ADDRESS, DEST_PORT_NO, log_fname_param);
}

void ReadTgtGEMROC_TIGER_GCfgReg (char*GCFGReg_setting_inst,unsigned int  GEMROC_ID_param, 
                                   unsigned int TIGER_ID_param, char*log_fname_param)
{
    g_reg_set_target_GEMROC(GEMROC_ID_param);
    g_reg_set_target_TIGER(TIGER_ID_param);
...
void WriteTgtGEMROC_TIGER_ChCfgReg_fromfile (char*ChCFGReg_setting_inst,unsigned int  GEMROC_ID_param, 
          unsigned int TIGER_ID_param, unsigned int channel_ID_param, char*ChCFGReg_def_fname_param, char*log_fname_param)
{   int i;
    ...
    if (channel_ID_param < 64)  {
        ...
        send_TIGER_Ch_CFG_Reg_CMD_PKT(TIGER_ID_param,COMMAND_STRING,ch_reg_command_words,sizeof(ch_reg_command_words),DEST_IP_ADDRESS,DEST_PORT_NO,log_fname_param) ; }
    else {
        for(i=0;i<64;i++)
       {
...
void ReadTgtGEMROC_TIGER_ChCfgReg (char*ChCFGReg_setting_inst, unsigned int GEMROC_ID_param, unsigned int TIGER_ID_param, 
                                    unsigned int channel_ID_param, unsigned int verbose_mode, char*log_fname_param)
{   int i;
    ch_reg_set_target_GEMROC(GEMROC_ID_param);
    ch_reg_set_target_TIGER(TIGER_ID_param);
    ch_reg_set_target_channel(channel_ID_param);
...
void Set_GEMROC_TIGER_ch_TPEn (char*ChCFGReg_setting_inst, unsigned int GEMROC_ID_param, unsigned int TIGER_ID_param,
          unsigned int  Channel_ID_param, unsigned int TP_disable_FE_param, unsigned int TriggerMode_param, char*log_fname_param)
{   int i;
    ch_reg_set_target_GEMROC(GEMROC_ID_param);
    ch_reg_set_target_TIGER(TIGER_ID_param);
    ch_reg_set_to_ALL_param (0);
...
void Set_Vth_T1 (char*ChCFGReg_setting_inst, unsigned int GEMROC_ID_param, unsigned int TIGER_ID_param, 
            unsigned int Channel_ID_param, unsigned int VthT1_param, char*log_fname_param)
{    int i;
    ch_reg_set_target_GEMROC(GEMROC_ID_param);
    ch_reg_set_target_TIGER(TIGER_ID_param);
    ch_reg_set_to_ALL_param (0);
...
void set_AVcasp_global(char*GCFGReg_setting_inst, unsigned int GEMROC_ID_param, unsigned int TIGER_ID_param,
                        unsigned int AVcasp_global_param, char*log_fname_param)
{
    g_reg_set_target_GEMROC(GEMROC_ID_param);
    g_reg_set_target_TIGER(TIGER_ID_param);
...
void SynchReset_to_TgtFEB(char*gemroc_DAQ_inst, unsigned int GEMROC_ID_param, unsigned int TargetFEB_param, unsigned int To_ALL_param)
{
    DAQ_set_target_GEMROC(GEMROC_ID_param);
    DAQ_set_target_TCAM_ID(TargetFEB_param, To_ALL_param);
    char*COMMAND_STRING = "CMD_GEMROC_DAQ_TIGER_SYNCH_RST";
    send_GEMROC_DAQ_CMD(GEMROC_ID_param, gemroc_DAQ_inst, COMMAND_STRING);
}
void SynchReset_to_TgtTCAM(char*gemroc_DAQ_inst, unsigned int GEMROC_ID_param, unsigned int TargetTCAM_param, unsigned int To_ALL_param)
{
    DAQ_set_target_GEMROC(GEMROC_ID_param);
...
void DAQ_set_and_TL_start(char*gemroc_DAQ_inst, unsigned int GEMROC_ID_param, 
...
void Set_OV_OC_OT_PWR_CUT_EN_FLAGS(char*gemroc_inst_param, unsigned int GEMROC_ID_param, unsigned int FEB_OVC_EN_pattern_param, 
       unsigned int FEB_OVV_EN_pattern_param, unsigned int FEB_OVT_EN_pattern_param,unsigned int ROC_OVT_EN_param, char*log_file_param)
{
...
void Set_OVVA_LIMIT(char*gemroc_inst_param, unsigned int GEMROC_ID_param, unsigned int FEB3_OVVA_thr_param, unsigned int FEB2_OVVA_thr_param,
             unsigned int  FEB1_OVVA_thr_param, unsigned int FEB0_OVVA_thr_param, char*log_file_param)
{
...
void Set_OVVD_LIMIT(char*gemroc_inst_param, unsigned int GEMROC_ID_param, unsigned int FEB3_OVVD_thr_param, unsigned int FEB2_OVVD_thr_param,
                           unsigned int  FEB1_OVVD_thr_param, unsigned int FEB0_OVVD_thr_param, char*log_file_param)
{   
...
void Set_OVCA_LIMIT(char*gemroc_inst_param,unsigned int GEMROC_ID_param, unsigned int FEB3_OVCA_thr_param, unsigned int FEB2_OVCA_thr_param,
                    unsigned int  FEB1_OVCA_thr_param, unsigned int FEB0_OVCA_thr_param, char*log_file_param)
{
...
void Set_OVCD_LIMIT(char*gemroc_inst_param, unsigned int GEMROC_ID_param, unsigned int FEB3_OVCD_thr_param, unsigned int FEB2_OVCD_thr_param,
                     unsigned int  FEB1_OVCD_thr_param,unsigned int  FEB0_OVCD_thr_param,char* log_file_param)
{
    float IADC_conv_fact_INA_GAIN_50 = 8.13;
    float IADC_conv_fact_INA_GAIN_200 = 8.13 / 4 ;
    float IADC_conv_fact_INA_GAIN;
...
void Set_OVTF_LIMIT(char*gemroc_inst_param, int GEMROC_ID_param, int FEB3_OVTF_thr_param,int FEB2_OVTF_thr_param, 
                     int FEB1_OVTF_thr_param, int FEB0_OVTF_thr_param, char*log_file_param)
{
...
void set_ROC_OVT_LIMIT(char*gemroc_inst_param, int GEMROC_ID_param, int ROC_OVT_thr_param, char*log_file_param)
{
...
void Read_GEMROC_LV_CfgReg(char*gemroc_inst_param,int GEMROC_ID_param,char* log_file_param)
{
    char*COMMAND_STRING = "CMD_GEMROC_LV_CFG_RD";
    send_GEMROC_LV_CMD(GEMROC_ID_param, COMMAND_STRING);
}
void Read_GEMROC_DAQ_CfgReg(char*gemroc_inst_param, int GEMROC_ID_param, char*log_file_param)
{
    char*COMMAND_STRING = "CMD_GEMROC_DAQ_CFG_RD";
    send_GEMROC_DAQ_CMD (GEMROC_ID_param, gemroc_inst_param, COMMAND_STRING);
}
...
//      sprintf(DEST_IP_ADDRESS,"192.168.1.%d",(GEMROC_ID+16)) ;
   ...
   fw = fopen("my_c_log.txt","w+");
    ...
    gemroc_update_command_words();
    send_GEMROC_CFG_CMD_PKT(GEMROC_ID,"CMD_GEMROC_LV_CFG_WR",gemroc_command_words,sizeof(gemroc_command_words),DEST_IP_ADDRESS,DEST_PORT_NO);
...
send_GEMROC_DAQ_CMD(GEMROC_ID, "gemroc_DAQ_XX","CMD_GEMROC_DAQ_CFG_WR");

g_reg_init(GEMROC_ID,"TIGER_def_g_cfg_2018.txt");
ch_reg_init(GEMROC_ID,"TIGER_def_ch_cfg_2018.txt");

GEMROC_IVT_read_and_log(GEMROC_ID,1, IVT_LOG_ENABLE, "IVT_log_file");
FEBPwrEnPattern_set(GEMROC_ID,1);
ResetTgtGEMROC_ALL_TIGER_GCfgReg(GEMROC_ID, "gemroc_DAQ_XX");
WriteTgtGEMROC_TIGER_GCfgReg_fromfile("g_inst", GEMROC_ID, 0, "TIGER_def_g_cfg_2018.txt", "log_file");
ReadTgtGEMROC_TIGER_GCfgReg("g_inst", GEMROC_ID, 0, "log_file" );
WriteTgtGEMROC_TIGER_ChCfgReg_fromfile("c_inst", GEMROC_ID,0, 64,"TIGER_def_ch_cfg_2018.txt", "log_file");
ReadTgtGEMROC_TIGER_ChCfgReg ("c_inst", GEMROC_ID,0,64, 0, "log_file" );
set_FE_TPEnable("g_inst", GEMROC_ID, 0, 1, "log_file");
Set_GEMROC_TIGER_ch_TPEn ("c_inst", GEMROC_ID,0,0,0,1,"log_file");
SynchReset_to_TgtFEB("gemroc_DAQ_XX", GEMROC_ID, 0, 0 );
SynchReset_to_TgtTCAM("gemroc_DAQ_XX", GEMROC_ID,0,0);
DAQ_set_and_TL_start("gemroc_DAQ_XX", GEMROC_ID,1,1);
